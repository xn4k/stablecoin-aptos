...
import { acceptOwnership } from "../../scripts/typescript/acceptOwnership";
...
describe("acceptOwnership", () => {
  ...
  it("should call the acceptOwnership function with correct inputs", async () => {
    ...
    const acceptOwnershipFn = sinon.fake();
    aptosExtensionsPackageStub.returns({
      ownable: {
        acceptOwnership: acceptOwnershipFn
      }
    });
    ...
    await acceptOwnership({
      ...
      stablecoinPackageId,
      newOwnerKey: newOwner.privateKey.toString(),
      rpcUrl
    });
    ...
    assert.strictEqual(
      acceptOwnershipFn.calledOnceWithExactly(newOwner, stablecoinAddress),
      true
    );
  });
});
